...
export async function POST(req: NextRequest) {
  try {
    ...
    // 既存PDFチェック（リトライ対応）
    const { data: existing } = await supabase.from('pdf_exports').select('file_url').eq('diagnosis_id', diagnosisId).single()
    ...
    // クレジット消費
    if (userId) {
      const cr = await consumeCredits(userId, CREDIT_COSTS.pdf, 'pdf_generate')
      if (!cr.success) {
        return NextResponse.json({ success: false, errorCode: 'INSUFFICIENT_CREDITS', error: `PDF生成には${CREDIT_COSTS.pdf}クレジット必要です`, remainingCredits: cr.remainingCredits }, { status: 402 })
      }
    }

    // 診断データ取得
    const { data: diagnosis } = await supabase.from('diagnoses').select('inputs, result').eq('id', diagnosisId).single()
    if (!diagnosis) return NextResponse.json({ success: false, errorCode: 'NOT_FOUND', error: '診断データが見つかりません' }, { status: 404 })
    ...
    return NextResponse.json({ success: true, downloadUrl, creditsUsed: CREDIT_COSTS.pdf })
  } catch (err) {
    console.error('PDF error:', err)
    return NextResponse.json({ success: false, errorCode: 'PDF_FAILED', error: err instanceof Error ? err.message : 'PDF生成に失敗しました' }, { status: 500 })
  }
}
